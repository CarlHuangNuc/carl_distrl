...
def call_qwen(client, system_msg, prompt, image_list, image_path):
    
    if type(prompt) == list:
        input_msg = [{"type": "text", "text": system_msg + "\n=====Examples====="}]
        for i in range(len(image_list)-1):
            input_msg.append({"type": "text", "text": "\nScreenshot:"})
            input_msg.append({"type": "image_url", "image_url": {"url": f"data:image/png;base64,{process_image(image_list[i])}"}})
            input_msg.append({"type": "text", "text": prompt[i]})

        input_msg.append({"type": "text", "text": "=====Your Turn====="})
        input_msg.append({"type": "text", "text": "\nScreenshot:"})
        input_msg.append({"type": "image_url", "image_url": {"url": f"data:image/png;base64,{process_image(image_path)}"}})
        input_msg.append({"type": "text", "text": prompt[-1]})

    else:
        input_msg = [
                        {"type": "text", "text": system_msg + "\n" + prompt},
                        {"type": "image_url", "image_url": {"url": f"data:image/png;base64,{process_image(image_path)}"}}
                    ]
...
class EndResultEvaluator:
    def __init__(self, gemini_key=None, task_set=None):
        ...
        #self.client = OpenAI(api_key="sk-3e9cf6e6052a4c7dba9dd02bc30bd529", base_url="https://dashscope.aliyuncs.com/compatible-mode/v1",)
        ## ali  cloud...

        self.client = OpenAI(api_key="sk_daisy", base_url="http://10.208.30.94:8000/v1",)
        ### yanzhe setup
    ...
    def _evaluate(self, intent: str, image_path: str) -> bool:
        ...
        response_text = call_qwen(self.client, system_msg, prompt, cot_image_list, image_path)
